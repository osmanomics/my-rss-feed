...
ABBVIE_PRODUCTS_FILE = "AbbVie_Products.xlsx"
NOT_TRACKED_CATEGORY = "NOT TRACKED"
NOT_TRACKED_ACTION = "Not tracked"

def load_abbvie_products():
    """Load the AbbVie Health Canada product portfolio from the Excel file.
    Returns a list of compiled regex patterns for case-insensitive whole-word matching.
    """
    if not os.path.exists(ABBVIE_PRODUCTS_FILE):
        print(f"WARNING: {ABBVIE_PRODUCTS_FILE} not found — AbbVie product filter disabled.")
        return []
    wb = openpyxl.load_workbook(ABBVIE_PRODUCTS_FILE)
    ws = wb.active
    terms = set()
    ...
    # Compile case-insensitive whole-word patterns sorted longest-first to
    # favour more specific matches (e.g. "REFRESH PLUS" before "REFRESH").
    patterns = [
        re.compile(r'\b' + re.escape(term) + r'\b', re.IGNORECASE)
        for term in sorted(terms, key=len, reverse=True)
    ]
    print(f"Loaded {len(patterns)} AbbVie product/ingredient terms from {ABBVIE_PRODUCTS_FILE}.")
    return patterns

def is_abbvie_relevant(title, summary, abbvie_patterns):
    """Return True if the title or summary mentions at least one AbbVie product/ingredient.
    If no patterns are loaded (file missing), defaults to True so nothing is silently dropped.
    """
    if not abbvie_patterns:
        return True
    text = f"{title} {summary}"
    return any(pattern.search(text) for pattern in abbvie_patterns)
...
def main():
    abbvie_patterns = load_abbvie_products()
    ...
    for url in RSS_URLS:
        ...
        for entry in feed.entries:
            ...
            # --- AbbVie Portfolio Gate -------------------------------------------
            # If the update doesn't mention any product in AbbVie's Health Canada
            # portfolio it cannot affect AbbVie from a regulatory affairs standpoint,
            # so we mark it NOT TRACKED immediately and skip the (costly) AI call.
            if not is_abbvie_relevant(title, summary, abbvie_patterns):
                print(f"  [NOT TRACKED] No AbbVie product detected — skipping AI: {title}")
                new_items.append({
                    "title": title,
                    "link": link,
                    "pubDate": entry.get('published', entry.get('updated', '')),
                    "category": NOT_TRACKED_CATEGORY,
                    "action": NOT_TRACKED_ACTION
                })
                continue
            # ---------------------------------------------------------------------
